...
export default function ConversationPage() {
  ...
  if (authLoading || isLoading) {
    return (
      <div className="min-h-screen pt-24 sm:pt-28 flex items-center justify-center bg-neutral-50">
  ...
  if (!chatUser) {
    return (
      <div className="min-h-screen pt-24 sm:pt-28 flex flex-col items-center justify-center bg-neutral-50 px-4">
        <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
          <p className="text-neutral-500 mb-4">User not found</p>
          <Link href="/messages">
  ...
  return (
    <div className="fixed inset-0 pt-[88px] sm:pt-24 flex flex-col bg-gradient-to-b from-neutral-100 to-neutral-50">
      ...
      <div className="flex-shrink-0 px-2 sm:px-4 pt-2 sm:pt-3">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl shadow-md border border-neutral-200/80 overflow-hidden">
            <div className="px-3 sm:px-5 py-3 sm:py-4">
              <div className="flex items-center gap-3 sm:gap-4">
                {/* Back button */}
                <button
                  onClick={() => router.push('/messages')}
                  className="p-2 rounded-xl text-neutral-500 hover:text-neutral-900 hover:bg-neutral-100 transition-all flex-shrink-0"
                >
                ...
                <Link href={`/profile/${chatUser.username}`} className="flex items-center gap-3 flex-1 min-w-0 group">
                  ...
                  <div className="min-w-0 flex-1">
                    <h2 className="font-semibold text-neutral-900 text-sm sm:text-base truncate group-hover:text-neutral-700 transition-colors">
                      {chatUser.displayName || chatUser.username}
                    </h2>
                    {/* Online status badge */}
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {isTyping ? (
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-neutral-100 rounded-full">
                          ...
                          <span className="text-xs text-neutral-600 font-medium">typing</span>
                        </span>
                      ) : chatUser.isOnline ? (
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-emerald-50 rounded-full">
                        ...
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-neutral-100 rounded-full">
                          ...
                          <span className="text-xs text-neutral-500 font-medium">Offline</span>
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-1 sm:gap-1.5 flex-shrink-0 bg-neutral-50 rounded-xl p-1">
                  <button
                    onClick={() => handleStartCall('voice')}
                    disabled={!chatUser.isOnline}
                    className="p-2.5 sm:p-3 rounded-xl text-neutral-600 hover:text-neutral-900 hover:bg-white hover:shadow-sm transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:shadow-none"
                  ...
                  <button className="p-2.5 sm:p-3 rounded-xl text-neutral-500 hover:text-neutral-700 hover:bg-white hover:shadow-sm transition-all">
      ...
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto overscroll-contain px-2 sm:px-4 py-3">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl shadow-md border border-neutral-200/80 min-h-full">
            <div className="px-3 sm:px-5 py-4 space-y-3">
              {Object.entries(groupedTimeline).map(([date, dateItems]) => (
                <div key={date}>
                  {/* Date separator */}
                  <div className="flex items-center justify-center my-4">
                    <span className="px-3 py-1.5 bg-neutral-100 border border-neutral-200 rounded-full text-xs text-neutral-500 font-medium">
                      {date}
                    </span>
                  </div>
                    ...
                    if (item.type === 'call') {
                      ...
                      return (
                        <div key={call._id} className="flex justify-center my-3">
                          <div className="flex items-center gap-3 px-4 py-2.5 bg-neutral-50 border border-neutral-200 rounded-full shadow-sm">
                            <div className={`p-1.5 rounded-full ${call.status === 'missed' || call.status === 'declined' ? 'bg-red-100' : 'bg-emerald-100'}`}>
                        ...
                        <div className="flex flex-col">
                          ...
                          <div
                            className={`max-w-[80%] sm:max-w-[70%] px-3 sm:px-4 py-2 sm:py-2.5 rounded-2xl transition-all duration-300 ${
                              isOwn
                                ? 'bg-neutral-900 text-white rounded-br-md shadow-md'
              ...
              {isTyping && (
                <div className="flex items-end gap-2 justify-start mb-4">
                  ...
                  <div className="bg-neutral-100 border border-neutral-200 px-3 sm:px-4 py-2.5 sm:py-3 rounded-2xl rounded-bl-md">
            ...
            <div className="px-3 sm:px-4 py-2.5 sm:py-3">
              <div className="flex items-center gap-2 sm:gap-3">
                <input
                  ...
                  className="flex-1 min-w-0 bg-neutral-50 border border-neutral-200 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base text-neutral-900 placeholder-neutral-400 focus:outline-none focus:ring-2 focus:ring-neutral-900/10 focus:border-neutral-300 transition-all"
                />
                <button
                  onClick={handleSend}
                  disabled={!newMessage.trim() || isSending}
                  className="p-2.5 sm:p-3 bg-neutral-900 rounded-xl text-white hover:bg-neutral-800 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm flex-shrink-0"
                >
